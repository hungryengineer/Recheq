...
export const repository = {
  ...
  getCaseByIdAndOrg: async (caseId: string, orgId: string) => {
    const [result] = await db.select().from(schema.cases).where(
       and(eq(schema.cases.id, caseId), eq(schema.cases.org_id, orgId))
    );
    return result || null;
  },
  ...
  updateCaseStatusAndVerdict: async (tx: any, caseId: string, status: CaseStatus, verdict: string, riskScore: number) => {
    await (tx || db).update(schema.cases)
  ...
  getConsentByCaseId: async (caseId: string) => {
    const [result] = await db.select().from(schema.consents).where(eq(schema.consents.case_id, caseId));
    return result || null;
  },
  getDocumentByCaseAndSha: async (caseId: string, sha256: string) => {
    const [result] = await db.select().from(schema.documents).where(
      and(eq(schema.documents.case_id, caseId), eq(schema.documents.sha256, sha256))
    );
    return result || null;
  },
  ...
  createExtraction: async (docId: string, args: any) => {
    const [result] = await db.insert(schema.extractions).values({
      document_id: docId,
  ...
  updateExtractionSuccess: async (id: string, data: any, usage: any) => {
    await db.update(schema.extractions).set({
      status: 'success',
      extracted_data: data,
      token_usage: usage,
  ...
  getSuccessfulExtractions: async (docIds: string[]) => {
    ...
    return await db.select().from(schema.extractions).where(
      and(inArray(schema.extractions.document_id, docIds), eq(schema.extractions.status, 'success'))
    );
  },
  ...
  updateRecordSuccess: async (id: string, history: EpfoHistory) => {
    await db.update(schema.epfoRecords).set({
      status: 'success',
