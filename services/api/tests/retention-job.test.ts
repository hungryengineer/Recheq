...
import { retentionJob, startWorkers } from '../src/workers/worker.js';
  ...
  return {
    schema: {
      cases: {
        id: 'cases.id',
        org_id: 'cases.org_id',
        candidate_name: 'cases.candidate_name',
    ...
    createDb: () => ({
      ...
      select: selectMock.mockReturnValue({
        from: fromMock.mockReturnValue({ where: whereSelectMock }),
      }),
    }),
  };
});
...
describe('retentionJob', () => {
  ...
  it('fetches old cases, deletes documents, and nullifies PII', async () => {
    ...
    whereSelectMock.mockResolvedValueOnce([{ id: 'case-1', org_id: 'org-1' }]);
