...
describe('verifyChain', () => {
  const event1: EventRecord = {
    id: 'e1', case_id: 'c1', seq: 1, kind: 'case_created',
    payload: { status: 'draft' }, actor: 'system',
    prev_hash: null, hash: '', created_at: new Date().toISOString()
  };
  event1.hash = calculateEventHash(event1.prev_hash, event1.seq, event1.kind, event1.payload);

  const event2: EventRecord = {
    id: 'e2', case_id: 'c1', seq: 2, kind: 'consent_granted',
    payload: { ip: '1.2.3.4' }, actor: 'user',
    prev_hash: event1.hash, hash: '', created_at: new Date().toISOString()
  };
