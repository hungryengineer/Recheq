...
async function verifyEnv(): Promise<void> {
  ...
  if (fs.existsSync(localEnv)) {
    ...
    const hasJwtSecret = /^JWT_SECRET=.+$/m.test(envContent);
    ...
    if (!hasJwtSecret) {
      ...
      // Append or replace the empty JWT_SECRET= line
      if (/^JWT_SECRET=\s*$/m.test(envContent)) {
        envContent = envContent.replace(/^JWT_SECRET=\s*$/m, `JWT_SECRET=${secret}`);
      } else {
