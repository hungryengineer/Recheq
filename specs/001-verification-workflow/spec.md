...
| Step succeeds with a null artifact                 | coerced to `not_assessed` — only `artifact === null` is coerced; empty arrays/objects remain `succeeded`                                                                                                                                                                                                                                                                                                   |
...
| Two steps declare the same output artifact         | output artifacts are addressed by step id (design D1): dependents reference producer ids, so the only expressible collision is a duplicate step id, rejected at construction. Sharing _inputs_ is allowed and expected — `doc.extract` and `doc.forensics` both consume the payslip/form16 documents — but shared input does not alias outputs: each step publishes its own artifact under its own id (§4) |
...
| Transient provider outage (rate limit/unavailable) | two distinct paths: (1) transient faults thrown mid-run raise `RecoverableWorkflowError` and the job is retried instead of failing the step; (2) a declared source reporting final unavailability (typed failure result) yields `not_assessed` per R1.16 — never an undeclared fallback                                                                                                                    |
